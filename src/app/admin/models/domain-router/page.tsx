      ...
      <Card className="p-6">
        ...
        <div className="space-y-6">
          <div className="space-y-2">
            <Label>
              Routing Threshold: {settings.routingThreshold.toFixed(2)}
            </Label>
            <Slider
              value={[settings.routingThreshold]}
              onValueChange={([value]) =>
                setSettings({ ...settings, routingThreshold: value })
              }
              min={0}
              max={1}
              step={0.05}
            ...
            <p className="text-sm text-muted-foreground">
              Minimum confidence to route to a domain
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Slider
              ...
              onValueChange={([value]) =>
                setSettings({ ...settings, maxDomains: value })
              }
              min={1}
              max={10}
              step={1}
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              ...
              <p className="text-sm text-muted-foreground">
                Use general domain when no match found
              </p>
            </div>
            <Switch
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              ...
              <p className="text-sm text-muted-foreground">
                Learn from successful routes over time
              </p>
            </div>
            <Switch
              checked={settings.enableRouteOptimization}
              onCheckedChange={(checked) =>
                setSettings({ ...settings, enableRouteOptimization: checked })
              }
            />
          </div>
        </div>

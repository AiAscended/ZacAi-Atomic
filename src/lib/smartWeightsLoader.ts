 ...
 */
export function discoverWeights(
  domainPath: string,
  domainName: string,
): WeightMetadata[] {
  ...
  const pretrainedPattern = new RegExp(
    `^${domainName}_pretrained_weights\\.json$`,
  );

  // Pattern: <domain>_trained_weights_v1_YYYY-MM-DD.json
  const trainedPattern = new RegExp(
    `^${domainName}_trained_weights_v(\\d+)_(\\d{4}-\\d{2}-\\d{2})\\.json$`,
  );
 ...
 */
export function selectBestWeights(
  weights: WeightMetadata[],
): WeightMetadata | null {
  if (weights.length === 0) return null;

  // Filter only existing files
  const existingWeights = weights.filter(
    (w) => w.exists && w.sizeBytes && w.sizeBytes > 0,
  );
 ...
 */
export function loadDomainWeights(
  domainPath: string,
  domainName: string,
  useCache: boolean = true,
): LoadedWeights | null {
...
export function getCacheStats() {
  return {
    ...
    domains: Array.from(weightsCache.keys()).map((k) =>
      path.basename(path.dirname(k)),
    ),
  };
}

...
export const customSnippets = (monaco: any) => ({
  typescript: [
    ...
    },
    {
      label: 'useState',
      kind: monaco.languages.CompletionItemKind.Snippet,
      insertText: 'const [${1:state}, set${1/(.*)/${1:/capitalize}/}] = useState$2($3);$0',
      insertTextRules: monaco.languages.CompletionItemInsertTextRule.InsertAsSnippet,
...
export function configureLanguageFeatures(monaco: any) {
  ...
  monaco.languages.registerCompletionItemProvider('typescript', {
    provideCompletionItems: (model: monaco.editor.IReadOnlyModel, position: monaco.Position) => {
      ...
      const snippets = customSnippets(monaco).typescript.map(s => ({ ...s, range }));
  ...
  monaco.languages.registerCompletionItemProvider('javascript', {
    provideCompletionItems: (model: monaco.editor.IReadOnlyModel, position: monaco.Position) => {
      ...
      const snippets = customSnippets(monaco).javascript.map(s => ({ ...s, range }));
      return { suggestions: snippets };
    },
  });

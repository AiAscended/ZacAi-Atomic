...
export function ThinkingSteps({ steps }: ThinkingStepsProps) {
  ...
  return (
    <div>
      ...
      >
        {expanded ? "Hide AI Thinking Steps" : `Show AI Thinking Steps (${steps.length})`}
      </button>
        ...
        <div className="space-y-2 rounded border border-indigo-400 bg-indigo-50 p-3 dark:bg-indigo-900 dark:border-indigo-600 text-xs font-mono overflow-auto max-h-60">
          {steps.map((step, idx) => (
            <div key={idx} className="border-l-2 border-indigo-600 pl-2">
              <div className="font-semibold text-indigo-700 dark:text-indigo-300">{step.description}</div>
              <div className="text-indigo-600 dark:text-indigo-400 opacity-70 text-xs">{step.timestamp}ms</div>

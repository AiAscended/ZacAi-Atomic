 ...
 */
export function rankResults(results: SearchResult[], parsedQuery: ParsedQuery): SearchResult[] {
  return results
    .map((result) => ({
      ...result,
 ...
 */
function calculateRelevanceScore(result: SearchResult, query: ParsedQuery): number {
  let score = 0
  ...
  for (const exclude of query.modifiers.exclude || []) {
    if (titleLower.includes(exclude.toLowerCase()) || snippetLower.includes(exclude.toLowerCase())) {
      score -= 10
    }
  }

...
export const documentationTokenizer = (text: string, opts?: { includeSystemTokens?: boolean }) => {
  ...
  if (opts?.includeSystemTokens ?? true) {
    const sys = ["<SYS_DOCUMENTATION>", "DOCUMENTATION_BASE"].filter((s) => DOCUMENTATION_CORE_TOKENS.includes(s))

...
const DOMAIN_DIR = path.join(process.cwd(), "src", "ai", "knowledge-domains", SECURITY_DOMAIN)
...
const registerSecurityDomain = () => {
  domainRegistry.registerDomain({
    ...
    description: "Security analysis, vulnerability detection, and secure coding guidance",
    atomicLevel: "organ",
    modules: SECURITY_MODULES,
    seedDataPath: path.join(DOMAIN_DIR, `${SECURITY_DOMAIN}_seeds`),
    learnedDataPath: path.join(DOMAIN_DIR, `${SECURITY_DOMAIN}_learned`),
    weightsPath: path.join(DOMAIN_DIR, `${SECURITY_DOMAIN}_weights`),
    enabled: true,
  })
}

export const securityInit = async (): Promise<void> => {
  registerSecurityDomain()

  try {
    const [seedEntries, vocabulary] = await Promise.all([
      loadSecuritySeedData(),
      loadSecuritySeedVocabulary(),
    ])

    if (!seedEntries.length) {
      console.warn("[Security] No seed data detected. Domain capabilities may be limited.")
    }

    console.log(
      `[Security] Vocabulary contains ${vocabulary.vulnerabilities.length} baseline vulnerabilities.`
    )
  } catch (error) {

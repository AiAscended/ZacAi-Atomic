...
export const getCodeReviewEmbeddingForTokens = (tokens: string[]) =>
  tokens.map(getCodeReviewEmbedding);

export const persistCodeReviewWeights = (weights: Record<string, number[]>) => {
  try {
    const content = JSON.stringify(
      {
        domain: CODE_REVIEW_DOMAIN,
        version: "0.2",
    ...
    updateFile(
      CODE_REVIEW_DOMAIN,
      "src/ai/knowledge-domains/code_review/code_review_weights/code_review_pretrained_weights.json",
      content,
    );
    return true;
  } catch (e) {

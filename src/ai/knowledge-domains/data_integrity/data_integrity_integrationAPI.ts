 ...
 */
async function loadSeedData(): Promise<DataIntegrityConcept[]> {
  try {
    ...
    console.log(
      `[DataIntegrity] Loaded ${allConcepts.length} concepts from ${jsonFiles.length} seed files`,
    );
    return allConcepts;
  } catch (error) {
...
async function query(prompt: string): Promise<any> {
  ...
  return {
    ...
    suggestion:
      "Consider data validation rules, consistency checks, and quality metrics",
    metadata: {
...
export const dataIntegrityInit = async () => {
  try {
    ...
    if (seedData.length === 0) {
      console.warn(
        "[DataIntegrity] No seed data loaded - domain may have limited capabilities",
      );
    }

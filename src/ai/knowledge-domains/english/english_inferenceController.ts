...
const definitions: Record<string, { definition: string; synonyms: string[]; example: string }> = {
  molecular: {
    ...
    example: "The molecular structure of water (H₂O) consists of two hydrogen atoms bonded to one oxygen atom.",
  },
  structure: {
    definition:
    ...
    synonyms: ["arrangement", "organization", "framework", "composition", "configuration"],
    example: "The structure of the building was designed to withstand earthquakes.",
  },
  atomic: {
    definition:
      "Relating to an atom or atoms; of or forming a single irreducible unit or component in a larger system.",
    synonyms: ["molecular", "microscopic", "nuclear", "elemental", "fundamental"],
    example: "The atomic number of carbon is 6, meaning it has 6 protons in its nucleus.",
  },
  "molecular structure": {
    definition:
      "The three-dimensional arrangement of atoms within a molecule, including the chemical bonds that hold the atoms together.",
    synonyms: ["molecular geometry", "molecular configuration", "chemical structure"],
    example: "Scientists use X-ray crystallography to determine the molecular structure of proteins.",
  },
}
...
export const englishRunInference = async (input: string, context?: any) => {
  ...
  if (lowerInput.match(/\b(what is|define|definition of|explain|tell me about|meaning of)\b/)) {
  ...
  if (lowerInput.match(/\b(synonym|synonyms|similar word|another word for)\b/)) {
    for (const [key, def] of Object.entries(definitions)) {
      if (lowerInput.includes(key)) {
        const response =
          `**Synonyms for "${key}":**\n\n` +
  ...
  if (lowerInput.match(/\b(use in a sentence|example sentence|sentence with)\b/)) {
    for (const [key, def] of Object.entries(definitions)) {
      if (lowerInput.includes(key)) {
        const response =

...
export const getEnglishEmbeddingForTokens = (tokens: string[]) =>
  tokens.map(getEnglishEmbedding);

export const persistEnglishWeights = (weights: Record<string, number[]>) => {
  try {
    const content = JSON.stringify(
      {
        domain: ENGLISH_DOMAIN,
        version: "0.2",
    ...
    updateFile(
      ENGLISH_DOMAIN,
      "src/ai/knowledge-domains/english/english_weights/english_pretrained_weights.json",
      content,
    );
    return true;
  } catch (e) {

...
import {
  loadAlgorithmsLearnedData,
  saveAlgorithmsLearnedData,
} from "./algorithms_learnedDataManager";

type AlgorithmsLearned = { notes: string[]; concepts: Record<string, unknown> };

export const algorithmsRunTrainingEpoch = async (opts?: {
  epochs?: number;
}) => {
  const data = (await loadAlgorithmsLearnedData()) as AlgorithmsLearned;
  const epoch = opts?.epochs ?? 1;
  data.notes.push(
    `algorithms trained ${epoch} epoch(s) at ${new Date().toISOString()}`,
  );

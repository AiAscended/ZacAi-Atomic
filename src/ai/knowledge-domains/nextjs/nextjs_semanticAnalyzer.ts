...
export function analyzeNextjsSemantics(input: string): NextjsSemanticAnalysis {
  ...
  if (lowerInput.includes("how") || lowerInput.includes("what") || lowerInput.includes("explain")) {
    ...
    intent = "implement"
    confidence = 0.9
  } else if (lowerInput.includes("migrate") || lowerInput.includes("upgrade") || lowerInput.includes("convert")) {
    intent = "migrate"
    confidence = 0.85
    ...
    intent = "debug"
    confidence = 0.85
  } else if (lowerInput.includes("deploy") || lowerInput.includes("production") || lowerInput.includes("build")) {
    intent = "deploy"
    confidence = 0.8
  }

  // Extract topics
  if (parseResult.metadata.routerType) topics.push(`${parseResult.metadata.routerType}-router`)
  if (parseResult.metadata.hasServerComponents) topics.push("server-components")
  if (parseResult.metadata.hasClientComponents) topics.push("client-components")
  ...
  const suggestedResponse = generateNextjsSuggestedResponse(intent, topics, complexity)

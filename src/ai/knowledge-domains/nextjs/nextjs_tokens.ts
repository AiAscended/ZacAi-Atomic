...
export const NEXTJS_CONCEPT_TOKENS: NextjsToken[] = NEXTJS_CONCEPTS.map(
  (concept) => ({
    id: tokenId++,
    text: concept,
...
export const NEXTJS_FEATURE_TOKENS: NextjsToken[] = NEXTJS_FEATURES.map(
  (feature) => ({
    id: tokenId++,
    text: feature,
...
export function getNextjsTokenByText(text: string): NextjsToken | null {
  return (
    ALL_NEXTJS_TOKENS.find(
      (token) => token.text.toLowerCase() === text.toLowerCase(),
    ) || null
  );
}

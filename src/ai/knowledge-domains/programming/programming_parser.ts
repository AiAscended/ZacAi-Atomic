import {
  tokenizeProgrammingInput,
  type TokenizedProgrammingInput,
} from "./programming_tokenizer";
...
export function parseProgrammingInput(input: string): ProgrammingParseResult {
  ...
  else if (lowerInput.includes("typescript") || lowerInput.includes("ts"))
    metadata.language = "typescript";
  else if (lowerInput.includes("python")) metadata.language = "python";

  // Detect paradigm
  if (lowerInput.includes("object oriented") || lowerInput.includes("oop"))
    metadata.paradigm = "object-oriented";
  else if (lowerInput.includes("functional")) metadata.paradigm = "functional";

  let type: ProgrammingParseResult["type"] = "general";

  if (lowerInput.includes("concept") || lowerInput.includes("what is"))
    type = "concept";
  else if (lowerInput.includes("syntax") || lowerInput.includes("how to write"))
    type = "syntax";
  else if (
    ...
    lowerInput.includes("design pattern") ||
    lowerInput.includes("architecture")
  )
    type = "design";
  else if (
    lowerInput.includes("algorithm") ||
    lowerInput.includes("complexity")
  )
    type = "algorithm";
  else if (
    lowerInput.includes("?") ||

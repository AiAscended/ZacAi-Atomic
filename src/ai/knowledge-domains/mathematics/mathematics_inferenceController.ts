...
function convertWordsToNumbers(input: string): string {
  ...
  converted = converted.replace(
    /(\w+)\s+hundred(?:\s+and)?\s+(\w+)/gi,
    (_match, hundreds, remainder) => {
...
export const mathematicsRunInference = async (
  ...
  const hasNumbers = lowerInput.match(
  ...
  const isAIMathQuery =
    ...
    (lowerInput.includes("equation") ||
      lowerInput.includes("mathematics") ||
...
`;
  ...
  for (const match of goesIntoMatches) {
    ...
    let dividendConverted = dividendWord
      ? convertWordsToNumbers(dividendWord)
      : null;
  ...
  if (expressionMatch && calculations.length === 0) {
    try {
      ...
      console.log(
        "[v0] ScientificCalculator evaluation failed, falling back to pattern matching",
      );
    }
  }

  if (calculations.length === 0) {
    const chainedMultMatches = Array.from(
      numericInput.matchAll(/(\d+)\s*[×x*]\s*(\d+)(?:\s*[×x*]\s*(\d+))+/gi),
    );
    for (const match of chainedMultMatches) {
      const fullMatch = match[0];
      const numbers = fullMatch
        .split(/[×x*]/)
      ...
      if (numbers.length >= 3) {
        ...
        for (let i = 1; i < numbers.length; i++) {
          ...
          steps.push(
            `Step ${i}: ${result / numbers[i]} × ${numbers[i]} = ${result}`,
          );
        }
    ...
    const addMultMatches = Array.from(
      numericInput.matchAll(
        /(\d+)\s*\+\s*(\d+)\s*[×x*]\s*(\d+)(?!\s*[+×x*])/gi,
      ),
    );
    for (const match of addMultMatches) {
      const [, num1, num2, num3] = match;
      if (calculations.some((c) => c.includes(`${num1} + ${num2} × ${num3}`)))
        continue;
    ...
    const simpleMultiplyMatches = Array.from(
      numericInput.matchAll(/(\d+)\s*[×x*]\s*(\d+)(?!\s*[×x*])/gi),
    );
    for (const match of simpleMultiplyMatches) {
      const [, num1, num2] = match;
      if (
        !calculations.some(
          (c) => c.includes(`${num1} ×`) || c.includes(`× ${num2}`),
        )
      ) {
    ...
    const simpleAddMatches = Array.from(
      numericInput.matchAll(/(\d+)\s*\+\s*(\d+)(?!\s*[×x*])/gi),
    );
    ...
    const divisionMatches = Array.from(
      numericInput.matchAll(/(\d+)\s*[÷/]\s*(\d+)/gi),
    );
    for (const match of divisionMatches) {
      ...
      if (divisor === 0) {
        calculations.push(
          `${num1} ÷ ${num2} = undefined (cannot divide by zero)`,
        );
      } else {
        const result = Number.parseInt(num1) / divisor;
        const isWholeNumber = result % 1 === 0;
        calculations.push(
          `${num1} ÷ ${num2} = ${isWholeNumber ? result : result.toFixed(2)}`,
        );
      }
    }
    ...
    if (powerMatch) {
      ...
      if (num1 === num2) {
        ...
        if (!isNaN(base) && !isNaN(exponent) && exponent > 0 && exponent < 20) {
          ...
          calculations.push(
            `${base} to the power of ${exponent} (${base}^${exponent}) = ${result.toLocaleString()}`,
          );
        }
      }
    }

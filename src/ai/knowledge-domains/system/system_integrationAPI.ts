...
async function loadSeedData(): Promise<SystemSeedEntry[]> {
  try {
    ...
    console.log(
      `[System] Loaded ${allConcepts.length} concepts from ${jsonFiles.length} seed files`,
    );
    return allConcepts;
  } catch (error) {
...
async function query(prompt: string): Promise<SystemQueryResult | null> {
  ...
  return {
    ...
    suggestion:
      "Review system configuration, dependencies, and deployment requirements",
    metadata: {
...
async function systemInit(): Promise<void> {
  try {
    ...
    if (seedData.length === 0) {
      console.warn(
        "[System] No seed data loaded - domain may have limited capabilities",
      );
    }

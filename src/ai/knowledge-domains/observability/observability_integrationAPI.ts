...
async function loadSeedData(): Promise<any[]> {
  try {
    ...
    console.log(
      `[Observability] Loaded ${allConcepts.length} concepts from ${jsonFiles.length} seed files`,
    );
    return allConcepts;
  } catch (error) {
...
async function query(prompt: string): Promise<any> {
  ...
  const matches = seedData.filter((concept) => {
    ...
    return observabilityKeywords.some((keyword) =>
  ...
  return {
    ...
    suggestion:
      "Consider implementing monitoring, logging, and alerting for better system visibility",
    metadata: {
...
export const observabilityInit = async () => {
  try {
    ...
    if (seedData.length === 0) {
      console.warn(
        "[Observability] No seed data loaded - domain may have limited capabilities",
      );
    }

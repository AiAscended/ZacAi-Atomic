...
export const generalRunInference = async (input: string, _context?: unknown) => {
  ...
  if (
    lowerInput.match(
      /\b(what is|define|definition of|explain|tell me about)\b/,
    ) &&
    lowerInput.match(
      /\b(scientific calculator|calculator|general knowledge|knowledge)\b/,
    )
  ) {
  ...
  if (!inferenceSucceeded || confidence < 0.4) {
    try {
      ...
      const searchQuery =
        queryKeywords.join(" ") || input.split(" ").slice(0, 5).join(" ");
      console.log(
        `[v0] ${GENERAL_DOMAIN} searching domain sources for:`,
        searchQuery,
      );
      ...
      for (const source of domainSources) {
        ...
        console.log(
          `[v0] ${GENERAL_DOMAIN} trying ${source.name} at: ${searchUrl}`,
        );
  ...
  if (
    ...
    if (
      ...
      lowerInput.includes("origin")
    ) {
      responseText =
        `I understand you're asking about the history of ${queryWords.join(", ")}. ` +
      ...
      responseText =
        `I recognize you're asking about ${queryWords.slice(0, 3).join(", ")}. ` +
      ...
      // Ultimate fallback
      responseText =
        `I received your question about "${input.substring(0, 50)}..." ` +
  ...
  return {
    ...
    metadata: {
      ...
      embeddingsUsed: embeddings.length > 0,
      sentimentDetected: sentiment?.sentiment || "neutral",
      inferenceMethod: sources.includes("Domain Inference (Trained Weights)")
        ? "trained_weights"
        : "url_lookup",
    },
  };

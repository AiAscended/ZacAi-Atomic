...
class SeedRegistryManager {
  ...
  public async loadAllSeeds(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `[SeedRegistry] Loaded ${this.stats.totalEntries} entries from ${this.stats.totalDomains} domains in ${this.stats.loadTimeMs}ms`,
      );
      console.log(
        `[SeedRegistry] Binary index size: ${this.binaryIndex.size} entries`,
      );
    } catch (error) {
  ...
  private async loadDomainSeeds(
    ...
    try {
      ...
      const jsonFiles = files.filter(
        (f) => f.endsWith(".json") && !f.includes("constants"),
      );
      ...
      if (domainEntries > 0) {
        console.log(
          `[SeedRegistry] ${domainName}: ${domainEntries} entries from ${jsonFiles.length} files`,
        );
      }
    } catch (error) {
      ...
      if ((error as { code: string }).code !== "ENOENT") {
        console.warn(
          `[SeedRegistry] Error loading ${domainName} seeds:`,
          error,
        );
      }
    }
   ...
   */
  private parseBinaryString(binaryString: string): {
    domainId: number;
    fileId: number;
    entryId: number;
  } {
  ...
  public lookup(key: string, domain?: string): SeedEntry | null {
    const lookupKey = domain
      ? `${domain}:${key.toLowerCase()}`
      : key.toLowerCase();
  ...
  public search(
    query: string,
    options?: { domain?: string; category?: string; tags?: string[] },
  ): SeedEntry[] {
  ...
  public async exportBinaryIndex(outputPath: string): Promise<void> {
    const index = Array.from(this.binaryIndex.entries()).map(
      ([key, binIndex]) => ({
        key,
...
if (process.env.NODE_ENV !== "test") {
  seedRegistry
    .loadAllSeeds()
    .catch((err) => console.error("[SeedRegistry] Auto-load failed:", err));
}

...
export class UnifiedModuleLoader {
  ...
  async loadAll(): Promise<void> {
    ...
    console.log(
      `🚀 Loading ${registry.enabledModules.length} enabled ${this.config.moduleType}(s)...`,
    );
    ...
    for (const moduleId of registry.enabledModules) {
      try {
        ...
        console.warn(
          `⚠️  Failed to load ${this.config.moduleType}: ${moduleId}`,
          error,
        );
      }
    }
    ...
    console.log(
      `✅ Loaded ${this.loadedModules.size} ${this.config.moduleType}(s)`,
    );
  }
  ...
  private async importModule(
    ...
    if (!entryPath) {
      throw new Error(
        `No entry point found for ${manifest.moduleType}: ${moduleId}`,
      );
    }
  ...
  private validateStructure(manifest: ModuleManifest): boolean {
    ...
    return !!(
      manifest.structure.hasSeedsFolder || manifest.structure.hasInference
    );
  }

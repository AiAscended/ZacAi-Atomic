...
export class ModuleScanner {
  ...
  async scanModules(): Promise<ModuleRegistry> {
    console.log(`🔍 Scanning ${this.config.moduleType}s in ${this.config.scanDir}...`);
    
    try {
      const entries = await fs.readdir(this.config.scanDir, { withFileTypes: true });
      ...
      for (const entry of entries) {
        ...
        try {
          ...
          console.warn(`⚠️  Failed to scan ${this.config.moduleType}: ${entry.name}`, error);
        }
      }
      ...
      console.log(`✅ Found ${this.registry.totalModules} ${this.config.moduleType}s`);
      
      return this.registry;
    } catch (error) {
      console.error(`❌ Failed to scan ${this.config.moduleType}s directory:`, error);
      throw error;
    }
  }
   ...
   */
  private async scanModuleDirectory(folderName: string): Promise<ModuleManifest | null> {
  ...
  private validateStructure(files: string[]): ModuleStructure {
    ...
    return {
      hasSeedsFolder: files.some(f => patterns.seeds?.test(f) || patterns.seedData?.test(f)),
      hasWeightsFolder: files.some(f => patterns.weights?.test(f) || f.includes("weight")),
      hasTokenizer: files.some(f => patterns.tokenizer?.test(f) || patterns.tokenizerConfig?.test(f)),
      ...
      hasTrainingPipeline: files.some(f => patterns.trainingPipeline?.test(f)),
      
      // Domain-specific
      hasInferenceController: files.some(f => patterns.inferenceController?.test(f)),
      hasTrainingController: files.some(f => patterns.trainingController?.test(f)),
  ...
  private buildPaths(files: string[]): ModulePaths {
    ...
    return {
      ...
      weightsPath: files.find(f => f === "weights" || f.startsWith("weights/")),
      pretrainedWeightsPath: files.find(f => patterns.pretrained?.test(f) && f.includes("weight")),
      finetunedWeightsPath: files.find(f => patterns.finetuned?.test(f) && f.includes("weight")),
      trainingWeightsPath: files.find(f => patterns.trainingWeights?.test(f)),
      learnedDataPath: files.find(f => patterns.learnedData?.test(f)),
      tokenizerPath: files.find(f => patterns.tokenizer?.test(f) && f.endsWith(".ts")),
      tokenizerConfigPath: files.find(f => patterns.tokenizerConfig?.test(f)),
      baseTokensPath: files.find(f => patterns.baseTokens?.test(f)),
      scriptsPath: files.find(f => f === "scripts" || f.startsWith("scripts/")),
      
      // Model-specific
      inferenceEnginePath: files.find(f => patterns.inferenceEngine?.test(f)),
      trainingPipelinePath: files.find(f => patterns.trainingPipeline?.test(f)),
      
      // Domain-specific
      inferenceControllerPath: files.find(f => patterns.inferenceController?.test(f)),
      trainingControllerPath: files.find(f => patterns.trainingController?.test(f)),
   ...
   * Gather metadata
   */
  private async gatherMetadata(modulePath: string, paths: ModulePaths): Promise<ModuleMetadata> {
    ...
    if (paths.baseTokensPath) {
      try {
        ...
        metadata.baseTokensCount = Array.isArray(tokens) ? tokens.length : Object.keys(tokens).length;
  ...
  private shouldEnableModule(structure: ModuleStructure): boolean {
    if (this.config.moduleType === "model") {
      ...
      return !!(structure.hasInferenceEngine && structure.hasSeedsFolder && structure.hasWeightsFolder);
    } else if (this.config.moduleType === "domain") {
      // Domains need inference controller and integration API
      return !!(structure.hasInferenceController && structure.hasIntegrationAPI);
    }
   ...
   */
  private detectModelType(folderName: string, files: string[]): ModuleManifest["modelType"] {
    ...
    if (patterns.transformer?.test(name) || patterns.transformer?.test(allFiles)) return "transformer";
    if (patterns.gan?.test(name) || patterns.gan?.test(allFiles)) return "gan";
    if (patterns.diffusion?.test(name) || patterns.diffusion?.test(allFiles)) return "diffusion";
    if (patterns.multimodal?.test(name) || patterns.multimodal?.test(allFiles)) return "multimodal";
...
export async function scanAndUpdate(config: ScannerConfig): Promise<ModuleRegistry> {
  const scanner = new ModuleScanner(config);
  const registry = await scanner.scanModules();
  await scanner.saveRegistry();
  return registry;

...
export class CodeLinter {
  ...
  public static lintCode(
    ...
    lines.forEach((line, index) => {
      ...
      if (/\bvar\s+/.test(line)) {
        issues.push({
          ...
          rule: "no-var",
          suggestion:
            "Replace 'var' with 'const' for immutable values or 'let' for mutable values",
        });
      }
    ...
    const errorCount = issues.filter(
      (i) => i.severity === LintSeverity.ERROR,
    ).length;
    const warningCount = issues.filter(
      (i) => i.severity === LintSeverity.WARNING,
    ).length;
    const infoCount = issues.filter(
      (i) => i.severity === LintSeverity.INFO,
    ).length;
    ...
    const score = Math.max(
      0,
      Math.min(100, 100 - (totalIssues / maxPenalty) * 100),
    );
   ...
   */
  private static checkMissingDocumentation(
    code: string,
    issues: LintIssue[],
  ): void {
    const lines = code.split("\n");
   ...
   */
  private static checkFunctionComplexity(
    code: string,
    issues: LintIssue[],
  ): void {
    const functionRegex =
    ...
    while ((match = functionRegex.exec(code)) !== null) {
      ...
      const functionBody = this.extractFunctionBody(
        afterFunction.substring(braceIndex),
      );

 ...
 */
export async function searchSources(domain: string, query: string): Promise<ScrapedContent[]> {
 ...
 */
export async function crawlDomainSources(domain: string): Promise<ScrapedContent[]> {
  const sources = findSources(domain)
  const results: ScrapedContent[] = []

...
export class ErrorHandler {
  ...
  private attemptRecovery(error: StructuredError): boolean {
    ...
    logger.info(`Attempting recovery for ${error.type}`, { attempt: attempts + 1 })
  ...
  private createFallbackResponse(
    ...
    switch (error.type) {
      case ErrorType.MODEL_ERROR:
        userMessage += "The AI model is temporarily unavailable. Please try again in a moment."
        break
      
      case ErrorType.DOMAIN_ERROR:
        userMessage += "I'm having trouble accessing specialized knowledge for this request."
        break
      
      case ErrorType.NETWORK_ERROR:
        userMessage += "There's a network connectivity issue. Please check your connection."
        break
      
      case ErrorType.TIMEOUT_ERROR:
        userMessage += "The request took too long to process. Try a shorter prompt."
        break
      
      case ErrorType.VALIDATION_ERROR:
        userMessage += "There's an issue with the input format. Please rephrase your request."
        break
      
      case ErrorType.INITIALIZATION_ERROR:
        userMessage += "The AI system is initializing. Please wait a moment and try again."
        break
    ...
    if (recoveryAttempted) {
      userMessage += " I've attempted to recover and can still help with simpler queries."
    }

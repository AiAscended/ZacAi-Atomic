...
export class MainOrchestrator {
  ...
  public async processPrompt(
    ...
    this.thinkingTracker.reset()
      ...
      logger.info("Processing prompt", {
        ...
        mode: (metadata as { mode?: string }).mode,
      })
  ...
  private shouldUseHybridMode(prompt: string, metadata: Record<string, unknown>): boolean {
    const config = this.config?.hybridMode
    if (!config?.enabled) {
      return false
    }

    const overrides = metadata as {
      mode?: string
      hybrid?: boolean
    ...
    if (overrides.forceClassic) return false
    if (overrides.mode === "hco" || overrides.hybrid === true) return true

    if (config.routingStrategy === "manual") {
      return false
    }

    const normalized = prompt.toLowerCase()
    const triggered = config.triggerWords.some((keyword) => normalized.includes(keyword.toLowerCase()))
    if (triggered) {
      return true
    }

    if (config.enforceCriticalPath) {
      const tags = (overrides.tags || []).join(" ").toLowerCase()
      if (tags.includes("executive") || tags.includes("audit")) {
        return true
      }
      const priority = overrides.priorityLabel?.toLowerCase()
      if (priority && (priority.includes("critical") || priority.includes("c-suite"))) {
        return true
      }
    }
  ...
  private async processViaHybridOrchestrator(
    prompt: string,
    sessionId: string,
    metadata: Record<string, unknown>,
    processingStart: number,
  ): Promise<OrchestratorResponse> {
    this.thinkingTracker.addStep("hco_route", "Routing prompt to Hybrid Cognitive Orchestrator", {
      routingStrategy: this.config?.hybridMode.routingStrategy,
      triggerWords: this.config?.hybridMode.triggerWords,
    })

    const hybridStart = Date.now()
    const hcoState = await hcoOrchestrator.execute(prompt, {
      userId: sessionId,
      metadata,
    })

    this.thinkingTracker.addStep("hco_complete", "Hybrid pipeline completed", {
      happiness: hcoState.happiness,
      decision: hcoState.reflection?.decision,
    })

    const summary = this.composeHybridSummary(hcoState)
    const confidence = this.deriveHCOConfidence(hcoState)
    const processingTime = Date.now() - processingStart
    const contentBlocks = this.composeHCOContentBlocks(hcoState)

    await this.learningMetricsTracker.recordInference({
      prompt,
      response: summary,
      confidence,
      domains: ["hybrid_cognitive"],
      modelsUsed: ["zacai-hco"],
      processingTime,
      tokensGenerated: summary.length,
      sessionId,
      timestamp: new Date().toISOString(),
    })
    ...
    enhancedMetricsCollector.recordInference({
      confidence,
      latency: Date.now() - hybridStart,
      success: true,
      domain: "hybrid_cognitive",
      model: "zacai-hco",
      tokensGenerated: summary.length,
    })
    ...
    return {
      ...
      metadata: {
        thinkingSteps: this.thinkingTracker.getSteps(),
        processingTime,
        modelsInvoked: ["zacai-hco"],
        domainsQueried: ["hybrid_cognitive"],
      },
      domains: ["hybrid_cognitive"],
      confidence,
      sources: hcoState.debate?.turns.map((turn) => `${turn.speaker}:${turn.confidence.toFixed(2)}`) ?? [],
      contentBlocks,
    }
  }

  private composeHybridSummary(state: HCOState): string {
    const lines: string[] = []
    if (state.reflection?.decision) {
      lines.push(`Reflection decision: ${state.reflection.decision}`)
    }
    if (state.empathy?.advisory) {
      lines.push(`Empathy advisory: ${state.empathy.advisory} (happiness ${state.happiness.toFixed(2)})`)
    }
    if (state.debate?.turns?.length) {
      const thesis = state.debate.turns.find((turn) => turn.speaker === "thesis")
      const antithesis = state.debate.turns.find((turn) => turn.speaker === "antithesis")
      lines.push(`Debate synthesis: ${state.debate.synthesis.text || thesis?.text || "n/a"}`)
      if (antithesis) {
        lines.push(`Counterpoint monitored: ${antithesis.text}`)
      }
    }
    if (state.hypotheses?.length) {
      lines.push(`Top hypothesis: ${state.hypotheses[0]?.text}`)
    }
    if (state.codeArtifact) {
      lines.push(`Prototype action: ${state.codeArtifact.action}`)
    }
    return lines.filter(Boolean).join("\n\n") || "Hybrid orchestrator completed without narrative output."
  }

  private composeHCOContentBlocks(state: HCOState) {
    const textBlocks = [
      {
        id: "hco-reflection",
        content: state.reflection?.decision || "No reflection decision produced",
      },
      {
        id: "hco-happiness",
        content: `Happiness index ${state.happiness.toFixed(2)} — ${state.empathy?.advisory ?? "no advisory"}`,
      },
    ]

    if (state.debate?.turns?.length) {
      const summary = state.debate.turns.map((turn) => `${turn.speaker.toUpperCase()}: ${turn.text}`).join(" | ")
      textBlocks.push({ id: "hco-debate", content: summary })
    }

    if (state.hypotheses?.length) {
      textBlocks.push({ id: "hco-hypothesis", content: `Primary hypothesis: ${state.hypotheses[0]?.text}` })
    }

    const codeBlocks = state.codeArtifact
      ? [
          {
            id: "hco-code",
            language: state.codeArtifact.language || "typescript",
            code: state.codeArtifact.body,
            filename: "hybridPrototype.ts",
          },
        ]
      : []
  ...
  private deriveHCOConfidence(state: HCOState): number {
    const base = state.happiness ?? 0.6
    const validationBoost = state.validations.filter((validation) => validation.valid).length * 0.03
    const empathyBoost = (state.empathy?.happiness ?? 0.5) * 0.1
    return Math.max(0.4, Math.min(0.98, base / 2 + validationBoost + empathyBoost))
  }

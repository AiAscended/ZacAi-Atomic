...
export class ResultsAggregator {
  private aggregationStrategies: Map<
    string,
    (results: ModelResult[]) => string
  >;
  ...
  public aggregate(
    ...
    const strategyFn =
      this.aggregationStrategies.get(strategy) ||
      this.aggregationStrategies.get("weighted")!;

    // Apply strategy
    const primaryContent = strategyFn(results);

    // Extract supplementary content
    const supplementaryContent = this.extractSupplementary(
      results,
      primaryContent,
    );
  ...
  private weightedMerge(results: ModelResult[]): string {
    ...
    for (let i = 1; i < sorted.length; i++) {
      if (sorted[i].confidence > 0.7) {
        const additional = this.extractUniqueInsights(
          merged,
          sorted[i].content,
        );
        if (additional) {
  ...
  private extractUniqueInsights(existing: string, newContent: string): string {
    ...
    const uniqueSentences = newSentences.filter((sentence) => {
      ...
      return !existingSentences.some((existing) =>
        this.areSimilar(existing, sentence),
      );
    });
  ...
  private extractSupplementary(
    ...
    for (const result of results) {
      if (result.content !== primaryContent) {
        const unique = this.extractUniqueInsights(
          primaryContent,
          result.content,
        );

...
export const getSecurityEmbeddingForTokens = (tokens: string[]) => tokens.map(getSecurityEmbedding)

export const persistSecurityWeights = (weights: Record<string, number[]>) => {
  try {
    const content = JSON.stringify(
      { domain: SECURITY_DOMAIN, version: "0.2", embeddingDim: EMBEDDING_DIM, seedWeights: weights },
      null,
      2,
    )

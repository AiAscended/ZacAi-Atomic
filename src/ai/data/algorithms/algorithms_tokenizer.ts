...
export const algorithmsTokenizer = (text: string, opts?: { includeSystemTokens?: boolean }) => {
  ...
  if (opts?.includeSystemTokens) {
    const sys = ["<SYS_ALGORITHMS>", "ALGORITHMS_BASE"].filter((s) => ALGORITHMS_CORE_TOKENS.includes(s))

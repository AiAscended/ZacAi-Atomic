...
export function analyzeProgrammingSemantics(input: string): ProgrammingSemanticAnalysis {
  ...
  if (lowerInput.includes("advanced") || lowerInput.includes("complex")) complexity = "advanced"
  else if (lowerInput.includes("intermediate") || topics.length > 2) complexity = "intermediate"

  const topicStr = topics.length > 0 ? topics.join(", ") : "programming concepts"

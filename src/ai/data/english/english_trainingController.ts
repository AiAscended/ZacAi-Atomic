...
import { loadEnglishLearnedData, saveEnglishLearnedData } from './english_learnedDataManager';
...
export const englishRunTrainingEpoch = async (opts?: { epochs?: number }) => {
  ...
  data.notes.push(`english trained ${epoch} epoch(s) at ${new Date().toISOString()}`);

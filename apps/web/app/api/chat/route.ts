...
import { registerAllDomains } from "@/ai/knowledge-domains/registerAllDomains";
...
async function ensureDomainsReady() {
  ...
  try {
    ...
    console.warn("[v0] ⚠️ Failed to register domains, using fallback", error);
    domainsReady = true;
    return;
  }
...
export async function POST(request: Request) {
  try {
    ...
    if (action === "initialize") {
      ...
      try {
        console.log(
          "[v0] Initializing full AI orchestrator with all domains...",
        );
    ...
    if (action === "chat") {
      ...
      if (!session) {
        ...
        console.log(
          "[v0] Using existing session with",
      ...
      try {
        console.log(
          "[v0] Calling real promptHandler.handlePrompt with full AI pipeline...",
        );
        ...
        console.log(
          "[v0] Session history updated, total messages:",
          session.history.length,
        );
        ...
        const errorText =
          "Sorry, something went wrong while processing your request. Please try again or check the Admin → Errors panel for details.";
        return NextResponse.json({

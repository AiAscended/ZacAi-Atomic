/**
 * File: src/ui/components/CodeBlock.tsx
...
export function CodeBlock({
  ...
  return (
    <div className={`relative rounded-lg border border-slate-700 bg-slate-900 text-slate-50 font-mono text-sm ${className ?? ""}`}>
      {filename && (
      ...
      <Button variant="ghost" size="icon" onClick={handleCopy} className="absolute top-2 right-2 z-20" aria-label="Copy code">
            ...
            <tbody>
              {lines.map((line, idx) => (
                <tr key={idx} className="align-top">
                  <td className="pr-2 text-right text-slate-500 select-none tabular-nums w-6">{idx + 1}</td>
                  <td dangerouslySetInnerHTML={{ __html: Prism.highlight(line, Prism.languages[language] || Prism.languages.javascript, language) }} />
                </tr>
              ))}
            </tbody>
          </table>

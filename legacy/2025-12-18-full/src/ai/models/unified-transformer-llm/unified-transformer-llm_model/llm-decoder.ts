...
export class LLMDecoder {
  ...
  forward(embeddings: number[][], useCausalMask: boolean = true): number[][] {
    ...
    const mask = useCausalMask
      ? this.createCausalMask(embeddings.length)
      : undefined;
   ...
   */
  setLayerWeights(
    layerWeights: Array<ReturnType<(typeof this.layers)[0]["getWeights"]>>,
  ): void {
    if (layerWeights.length !== this.numLayers) {
      throw new Error(
        `Layer count mismatch: expected ${this.numLayers}, got ${layerWeights.length}`,
      );
    }

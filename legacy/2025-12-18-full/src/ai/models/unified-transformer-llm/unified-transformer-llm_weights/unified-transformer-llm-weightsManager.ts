...
export class LLMWeightsManager {
  ...
  constructor(weightsDir?: string) {
    this.weightsDir =
      weightsDir || path.join(__dirname, "../../../data/weights/llm");
  }
  ...
  async loadWeights(
    ...
    try {
      ...
      console.log(
        `   - Last loss: ${weights.metadata.lastLoss?.toFixed(4) || "N/A"}`,
      );
  ...
  initializeWeights(config: LLMModelConfig): ModelWeights {
    ...
    const embeddings = this.initializeMatrix(
      config.vocabSize,
      config.embeddingDim,
    );
    ...
    for (let i = 0; i < config.numLayers; i++) {
      decoderLayers.push({
        // Attention weights
        Wq: this.initializeAttentionWeights(
          config.numHeads,
          config.embeddingDim,
          headDim,
        ),
        Wk: this.initializeAttentionWeights(
          config.numHeads,
          config.embeddingDim,
          headDim,
        ),
        Wv: this.initializeAttentionWeights(
          config.numHeads,
          config.embeddingDim,
          headDim,
        ),

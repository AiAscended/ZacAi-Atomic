...
import { loadErrorDetectionLearnedData, saveErrorDetectionLearnedData } from "./error_detection_learnedDataManager"

type ErrorDetectionLearned = { notes: string[]; concepts: Record<string, unknown> }

export const errorDetectionRunTrainingEpoch = async (opts?: { epochs?: number }) => {
  const data = (await loadErrorDetectionLearnedData()) as ErrorDetectionLearned
  const epoch = opts?.epochs ?? 1
  data.notes = data.notes || []
  data.notes.push(`error_detection trained ${epoch} epoch(s) at ${new Date().toISOString()}`)

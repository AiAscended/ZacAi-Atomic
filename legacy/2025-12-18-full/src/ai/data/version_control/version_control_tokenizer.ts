...
export const versionControlTokenizer = (text: string, opts?: { includeSystemTokens?: boolean }) => {
  ...
  if (opts?.includeSystemTokens) {
    const sys = ["<SYS_VERSION_CONTROL>", "VERSION_CONTROL_BASE"].filter((s) => VERSION_CONTROL_CORE_TOKENS.includes(s))

...
import {
  loadDocumentationLearnedData,
  saveDocumentationLearnedData,
} from "./documentation_learnedDataManager";

type DocumentationLearned = {
  notes: string[];
  concepts: Record<string, unknown>;
};

export const documentationRunTrainingEpoch = async (opts?: {
  epochs?: number;
}) => {
  ...
  data.notes.push(
    `documentation trained ${epoch} epoch(s) at ${new Date().toISOString()}`,
  );

...
export const getScienceEmbeddingForTokens = (tokens: string[]) =>
  tokens.map(getScienceEmbedding);
...
export const persistScienceWeights = (weights: Record<string, number[]>) => {
  try {
    ...
    updateFile(
      SCIENCE_DOMAIN,
      "src/ai/knowledge-domains/science/science_weights/science_pretrained_weights.json",
      content,
    );
    return true;
  } catch (e) {
...
export default {
  getScienceEmbedding,
  getScienceEmbeddingForTokens,
  persistScienceWeights,
};
